...
// KYBVerifyRequest holds parameters for a business verification (POST /kyb). A
// Document is required; Profile is optional.
type KYBVerifyRequest struct {
	...
	Profile  string // optional KYC profile to apply to the verification
}

// Verify verifies a business from its registration/incorporation document
// (POST /kyb). A document is required; an optional profile selects the KYC
// profile. It extracts the company details, checks official company registries,
// screens against sanctions/PEP watchlists, and returns directors and owners to
// verify. It returns the decoded JSON response, or an error.
func (k *KYBService) Verify(req KYBVerifyRequest) (map[string]any, error) {
	if req.Document == "" {
		return nil, invalid("A business document (image or PDF) is required.")
	}
	...
	putStr(payload, "profile", req.Profile)
